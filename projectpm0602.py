...
def loadDB() :
    ...
    con = pymysql.connect(host=IP_ADDR, user='root', password='1234', database='photoDB', charset='utf8')
    cur = con.cursor()

    sql = "SELECT id, fName, fType FROM photoTBL"  # ID로 추출하기
    cur.execute(sql)
...
def scaleCV2() :
    ...
    global outImageR, outImageG, outImageB, filename, photo, cvPhoto
    ####### CV2 메소드로 구현하기 --> photo2로 넘기기 ####
    scale = askfloat('스케일 계수', '확대값-->', minvalue = 1, maxvalue = 10)
    cvPhoto2 = cvPhoto[:]  # 복사
...
#     cvPhoto2 = cvPhoto[:]
#
#     hist, bins = np.histogram(cvPhoto2.flatten(), 256, [0, 256])
#
#     cdf = hist.cumsum()
#
...
#
#     img2 = cdf[cvPhoto2]
#     plt.subplot(121), plt.imshow(cvPhoto2), plt.title('Original')
#     plt.subplot(122), plt.imshow(cvPhoto2), plt.title('Equalization')
#     plt.show()
#
def equalizeHistCV2(): ## https://webnautes.tistory.com/1043
...
rawImage1Menu.add_command(label="엠보싱", command=embossing)
...
openCVMenu.add_command(label="GrayScale(OpenCV)", command=greyscaleCV2)
openCVMenu.add_command(label="블러링(OpenCV)", command=blurringCV2)
openCVMenu.add_separator()
openCVMenu.add_command(label="회전", command=rotateCV2)
openCVMenu.add_command(label="확대", command=scaleCV2)
openCVMenu.add_separator()
openCVMenu.add_command(label="수직 웨이브", command=waveVirCV2)
openCVMenu.add_command(label="수평 웨이브", command=waveHorCV2)
openCVMenu.add_separator()
openCVMenu.add_command(label="카툰화", command=cartoonCV2)

# ##############################################
machineCVMenu = Menu(mainMenu)
mainMenu.add_cascade(label="OpenCV", menu=machineCVMenu)
openCVMenu.add_separator()
machineCVMenu.add_command(label="이미지 열기", command=openOpenCV)
machineCVMenu.add_command(label="얼굴인식(머신러닝)", command=faceCV2)
machineCVMenu.add_command(label="얼굴가면 씌우기(머신러닝)", command=maskCV2)
machineCVMenu.add_command(label="평활화(머신러닝)", command=equalizeHistCV2)
